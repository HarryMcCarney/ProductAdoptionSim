...
module State =
    ...
    let Init() = Map ["colCount", 5; "rowCount", 5; "peopleCount", 100; "adoptorStartCount", 0; "running",0; "ticks", 0; ]

    let stateStore = Store.make (Init() )

    let getState k = 
        (Store.get stateStore)[k]
...
module Population =
    ...
    let adoptorThreholdMet p t =
        if (p |> Seq.filter(fun x -> x.Status = Adoptor) |> Seq.length) >= t then true 
        else false 

    let convertAdoptors (people: People) : People = 
        people 
        |> Array.groupBy(fun x -> x.Coordinates)
        |> Array.map(fun (co, p) -> if (adoptorThreholdMet p 3) then p |> Array.map(fun x -> {
                                                                                            Coordinates = co
                                                                                            Status = Adoptor 
                                                                                            })
                                    else p
    ...
    let convertToTrackingObject (people: People): array<PopulationTracker.Person> =
        people 
        |> Array.map(fun x-> {Status = (if x.Status = Prospect then PopulationTracker.Prospect else PopulationTracker.Adoptor); Coordinates = x.Coordinates})
    
    let updatePopulation  rows columns (people: People)  = 
        let tick = getState "ticks"
        people 
        |> convertToTrackingObject
        |> logPeople tick
        |> ignore

        people 
        |> convertAdoptors
        |> movePopulation rows columns
    ...
    let updateStore() = 
        Store.set peopleStore (     
                                ...
                                else (Store.get peopleStore)
                                )
                                        
    let setMarketingSpend (state:IStore<Map<string, int>>)  adoptors =
        state <~=  (fun m -> m.Change("adoptorStartCount", (fun _ -> Some adoptors)))
        peopleStore <~= (fun ps -> ps |> Array.mapi(fun i p -> if i< adoptors then {Coordinates = p.Coordinates; Status = Adoptor} 
                                                                    else p))
